...
bool WmfParser::load(const QByteArray& array)
{
    ...
    if (pheader.key == (quint32)APMHEADER_KEY) {
        ...
        mPlaceable = true;
#if DEBUG_RECORDS
        qDebug(kowmf) << "Placeable header!  Yessss!";
#endif
        ...
        if (pheader.checksum != checksum) {
            qCWarning(kowmf) << "Checksum for placeable metafile header is incorrect ( actual checksum" << pheader.checksum << ", expected checksum" << checksum << ")";
            return false;
        }
...
#if DEBUG_RECORDS
        qDebug(kowmf) << "bounding box in header: " << mBBoxLeft << mBBoxTop << mBBoxRight << mBBoxBottom
    ...
    if (((header.headerSize == 9) && (header.numOfParameters == 0)) || (mPlaceable)) {
        ...
        qCDebug(kowmf) << "WmfParser : incorrect file format !";
    }
...
#if DEBUG_RECORDS
        qDebug(kowmf) << "bounding box created by going through all records: "
...
{
    if (!(mValid)) {
        qDebug(kowmf) << "WmfParser::play : invalid WMF file";
        return false;
    }
...
#if DEBUG_RECORDS
        if ((mStandard)) {
            qDebug(kowmf) << "Standard :" << mBBoxLeft << ""  << mBBoxTop << ""  << mBBoxRight - mBBoxLeft << ""  << mBBoxBottom - mBBoxTop;
        } else {
            qDebug(kowmf) << "DPI :" << mDpi;
            qDebug(kowmf) << "size (inch):" << (mBBoxRight - mBBoxLeft) / mDpi
                          << "" << (mBBoxBottom - mBBoxTop) / mDpi;
            qDebug(kowmf) << "size (mm):" << (mBBoxRight - mBBoxLeft) * 25.4 / mDpi
                          << "" << (mBBoxBottom - mBBoxTop) * 25.4 / mDpi;
        }
        qDebug(kowmf) << mValid << "" << mStandard << "" << mPlaceable;
#endif
    }
    ...
    if (m_backend->begin(bbox)) {
        ...
        while ((recordType) && (!mStackOverflow)) {
...
#if DEBUG_RECORDS
            qDebug(kowmf) << "Record = " << koWmfFunc[ index ].name
                          << " (" << hex << recordType
                          << ", index" << dec << index << ")";
#endif
            ...
            case (META_SETBKMODE & 0xff):
                {
                    ...
                    stream >> bkMode;
                    //qDebug(kowmf) << "New bkMode: " << bkMode;
            ...
            case (META_SETMAPMODE & 0xff):
                {
                    stream >> mMapMode;
                    //qDebug(kowmf) << "New mapmode: " << mMapMode;
            ...
            case (META_SCALEWINDOWEXT & 0xff):
                {
                    ...
                    //qDebug(kowmf) <<"WmfParser::ScaleWindowExt :" << widthDenum <<"" << heightDenum;
                }
                break;
            ...
            case (META_RECTANGLE & 0xff):
                {
                    ...
                    stream >> bottom >> right >> top >> left;
                    //qDebug(kowmf) << left << top << right << bottom;
            ...
            case (META_PATBLT & 0xff):
                {
                    ...
                    //qDebug(kowmf) << "patBlt record" << hex << rasterOperation << dec
                    //              << x << y << width << height;
            ...
            case (META_SELECTOBJECT & 0xff):
                {
                    ...
                    if ((idx < mNbrObject) && (mObjHandleTab[ idx ] != 0))
                        ...
                        qDebug(kowmf) << "WmfParser::selectObject : selection of an empty object";
                }
                break;
...
#if DEBUG_RECORDS
                    qDebug(kowmf) << "text at" << x << y << "length" << stringLength
                                  << ':' << text;
                    //qDebug(kowmf) << "flags:" << hex << fwOpts << dec;
                    qDebug(kowmf) << "flags:" << fwOpts;
                    qDebug(kowmf) << "record length:" << size;
#endif
                    m_backend->drawText(mDeviceContext, x, y, text);
                }
                break;
            ...
            case (META_DIBBITBLT & 0xff):
                {
                    ...
                    if (size > 11) {      // DIB image
                        ...
                        qDebug(kowmf) << "WmfParser::dibBitBlt without image not implemented";
                    }
                }
                break;
            case (META_DIBSTRETCHBLT & 0xff):
                {
                    ...
                    if (dibToBmp(bmpSrc, stream, (size - 13) * 2)) {
                        ...
                        bmpSrc = bmpSrc.scaled(widthDst, heightDst);
            ...
            case (META_DIBCREATEPATTERNBRUSH & 0xff):
                {
                    ...
                    if (addHandle(handle)) {
                        ...
                        if (dibToBmp(bmpSrc, stream, (size - 5) * 2)) {
                            ...
                            qDebug(kowmf) << "WmfParser::dibCreatePatternBrush : incorrect DIB image";
                        }
                    }
                }
                break;
            ...
            case (META_CREATEPENINDIRECT & 0xff):
                {
                    ...
                    if (addHandle(handle)) {
                        ...
                        if (penStyle < 7)
                            ...
                            qDebug(kowmf) << "WmfParser::createPenIndirect: invalid pen" << style;

                        quint16 capStyle = (style & PenCapMask) >> 8;
                        if (capStyle < 3)
                            handle->pen.setCapStyle(koWmfCapStylePen[ capStyle ]);
                        else
                            qDebug(kowmf) << "WmfParser::createPenIndirect: invalid pen cap style" << style;

                        quint16 joinStyle = (style & PenJoinMask) >> 12;
                        if (joinStyle < 3)
                            handle->pen.setJoinStyle(koWmfJoinStylePen[ joinStyle ]);
                        else
                            qDebug(kowmf) << "WmfParser::createPenIndirect: invalid pen join style" << style;

                        handle->pen.setColor(qtColor(color));
                        handle->pen.setWidth(width);
                        qDebug(kowmf) << "Creating pen" << handle->pen;
                    }
                }
                break;
            case (META_CREATEFONTINDIRECT & 0xff):
                {
                    ...
                    if (addHandle(handle)) {
                        ...
                        //qDebug(kowmf) << height << width << weight << property;
                        ...
                        //qDebug(kowmf) << "Font height:" << height;
                    ...
                    if (addHandle(handle)) {
                        ...
                        if (sty == 2) {
                            if (arg2 < 6)
                                ...
                                qDebug(kowmf) << "WmfParser::createBrushIndirect: invalid hatched brush" << arg2;
                            ...
                            if (sty < 9)
                                ...
                                qDebug(kowmf) << "WmfParser::createBrushIndirect: invalid brush" << sty;
                                style = Qt::SolidPattern;
                            }
                        }
...
void WmfParser::createBoundingBox(QDataStream &stream)
{
    ...
    while (recordType) {
        ...
        if (size == 0) {
            qDebug(kowmf) << "WmfParser: incorrect file!";
            mValid = 0;
            return;
        }
...
#if DEBUG_BBOX
                qDebug(kowmf) << "setWindowOrg" << windowOrgX << windowOrgY;
#endif
                if (!windowExtIsSet)
                    break;
...
#if DEBUG_BBOX
                qDebug(kowmf) << "setWindowExt" << windowWidth << windowHeight
...
#if DEBUG_BBOX
                qDebug(kowmf) << "setViewportOrg" << viewportOrgX << viewportOrgY;
#endif
                orgX = viewportOrgX;
                orgY = viewportOrgY;
                if (viewportExtIsSet) {
...
#if DEBUG_BBOX
                qDebug(kowmf) << "setViewportExt" << viewportWidth << viewportHeight;
#endif
                orgX = viewportOrgX;
                orgY = viewportOrgY;
                extX = viewportWidth;
...
#if DEBUG_BBOX
            qDebug(kowmf) << "drawing record: " << (recordType & 0xff);
#endif
...
#if DEBUG_BBOX
            qDebug(kowmf) << "Recalculating BBox";
#endif
...
#if DEBUG_BBOX
            qDebug(kowmf) << orgX << orgY << extX << extY;
#endif
...
#if DEBUG_BBOX
        if (isOrgOrExt) {
            qDebug(kowmf) << "              mBBoxTop = " << mBBoxTop;
            qDebug(kowmf) << "mBBoxLeft = " << mBBoxLeft << "  mBBoxRight = " << mBBoxRight;
            qDebug(kowmf) << "           MBBoxBotton = " << mBBoxBottom;
            qDebug(kowmf) << "Max width,height = " << mMaxWidth << mMaxHeight;
        }
#endif
...
bool WmfParser::addHandle(KoWmfHandle* handle)
{
    ...
    if (idx < mNbrObject) {
        ...
        delete handle;
        mStackOverflow = true;
        qDebug(kowmf) << "WmfParser::addHandle : stack overflow = broken file !";
        return false;
    }
}
...
void WmfParser::deleteHandle(int idx)
{
    if ((idx < mNbrObject) && (mObjHandleTab[idx] != 0)) {
        ...
        qDebug(kowmf) << "WmfParser::deletehandle() : bad index number";
    }
}
...
//    if ( !bmp.loadFromData( (const uchar*)bmpHeader, pattern.size(), "BMP" ) ) {
    if (!bmp.loadFromData(pattern, "BMP")) {
        qDebug(kowmf) << "WmfParser::dibToBmp: invalid bitmap";
        return false;
    } else {
        return true;
    }
