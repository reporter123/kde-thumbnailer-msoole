...
#if DEBUG_WMFPAINT
    qDebug(kowmf) << "Using QPainter: " << mPainter->pen() << mPainter->brush() 
...
void WmfPainterBackend::restore()
{
    if (mSaveCount > 0) {
        ...
        qCDebug(kowmf) << "restore(): try to restore painter without save";
    }
...
void WmfPainterBackend::recalculateWorldTransform()
{
...
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "Scale for Window -> Viewport"
...
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "Only one of Window or Viewport set: scale window -> viewport = 1";
#endif
    }
    ...
    if (flip) {
        //qDebug(kowmf) << "Flipping round midpoint" << midpointX << midpointY << scaleX << scaleY;
        mWorldTransform.translate(midpointX, midpointY);
        mWorldTransform.scale(scaleX, scaleY);
        mWorldTransform.translate(-midpointX, -midpointY);
        //qDebug(kowmf) << "After flipping for window" << mWorldTransform;
    }
    ...
    //qDebug(kowmf) << "After window viewport calculation" << mWorldTransform;
    ...
    //qDebug(kowmf) << "Output transform" << mOutputTransform;
    //qDebug(kowmf) << "Total  transform" << newMatrix;
...
void WmfPainterBackend::setWindowOrg(int left, int top)
{
    ...
    qDebug(kowmf) << "Window rect: " << windowRect;
    qDebug(kowmf) << "Bounding rect: " << boundingRect();
#endif
}
...
#if DEBUG_WMFPAINT
    qDebug(kowmf) << x << ", " << y << " using " << mPainter->pen();
#endif
...
void WmfPainterBackend::drawPolyPolygon(WmfDeviceContext &context, QList<QPolygon>& listPa)
{
    ...
    // fill polygons
    if (brush != Qt::NoBrush) {
        //qDebug(kowmf) << "Filling polygon with " << brush;
...
{
#if DEBUG_WMFPAINT
    qDebug(kowmf) << x << y << width << height << hex << rasterOperation << dec;
#endif
...
void WmfPainterBackend::drawText(WmfDeviceContext &context, int x, int y, const QString& text)
{
...
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "Using current position:" << x << y;
#endif
    }
...
#if DEBUG_WMFPAINT
    qDebug(kowmf) << "font = " << mPainter->font() << " pointSize = " << mPainter->font().pointSize()
    ...
    //qDebug(kowmf) << "fontheight:" << context.height << "height:" << height << "y" << y;
...
void WmfPainterBackend::updateFromDeviceContext(WmfDeviceContext &context)
{
...
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "*** Setting fill brush to" << context.brush;
#endif
    }
    // FIXME: context.image
...
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "*** Setting font to" << context.font;
#endif
    }
    if (context.changedItems & DCPalette) {
        // NYI
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "*** Setting palette (NYI)";
#endif
    }
	...
	//Tired of esoteric compiler error this triggers. We don't use this library for printer output anyway.
	  ...
	  if (width == 1)
        ...
        mPainter->setPen(p);
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "*** Setting pen to" << p;
#endif
    }
    if (context.changedItems & DCClipRegion) {
        // Not used until SETCLIPREGION is used
#if DEBUG_WMFPAINT
        //qDebug(kowmf) << "*** region changed to" << context.region;
#endif
    }
...
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "*** Setting background text color to" << context.backgroundColor;
#endif
    }
    //----------------------------------------------------------------
    ...
    if (context.changedItems & DCBgMixMode) {
        ...
        mPainter->setBackgroundMode(context.bgMixMode == TRANSPARENT ? Qt::TransparentMode
                                                                     : Qt::OpaqueMode);
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "*** Setting background mode to" << context.bgMixMode;
#endif
    }
...
#if DEBUG_WMFPAINT
        qDebug(kowmf) << "*** Setting composition mode to" << context.rop;
#endif
    }
