...
void WmfDeviceContext::recalculateWorldTransform()
{
    ...
    if (flip) {
        qCDebug(kowmf) << "Flipping round midpoint" << midpointX << midpointY << scaleX << scaleY;
        m_worldTransform.translate(midpointX, midpointY);
        m_worldTransform.scale(scaleX, scaleY);
        m_worldTransform.translate(-midpointX, -midpointY);
        //qDebug(kowmf) << "After flipping for window" << m_worldTransform;
    }
    ...
    //qDebug(kowmf) << "After window viewport calculation" << m_worldTransform;
}
